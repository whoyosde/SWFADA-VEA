...
public class MyPage extends PageObject {
    ...
    public void validarDocumentoIncorporado() {
        WebElement btnModificar = getDriver().findElement(By.xpath("//p[contains(text(),'MODIFICAR')]"));
        WebDriverWait wait = new WebDriverWait(getDriver(), 60);
        wait.until(ExpectedConditions.visibilityOf(btnModificar));
        JavascriptExecutor jsExecutor = (JavascriptExecutor) getDriver();
        jsExecutor.executeScript("arguments[0].scrollIntoView({block: 'center', inline: 'center'});", btnModificar);
        if (btnModificar.isDisplayed()) {
            System.out.println("Se cargó correctamente el documento");
        } else {
            throw new AssertionError("El documento no se adjunto correctamente");
        }
    }
}
