...
public class MyPage extends PageObject {
    ...
    @FindBy(xpath = "//fa-icon[@alt=\"faEraser\"]")
    ...
    public void pulsarIconoEliminarFirma() {
    ...
    public void pulsarBotónBorrar() {
        WebDriverWait wait = new WebDriverWait(getDriver(), 30);
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h4[contains(text(),'Borrar firma de documento')]")));
        btnBorrar.waitUntilClickable();
        btnBorrar.click();
    }

    public void validarFirmaEliminada() {
        WebDriverWait wait = new WebDriverWait(getDriver(), 80);
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//p[contains(text(),'AUTORIZACIÓN')]")));

        wait.until(ExpectedConditions.invisibilityOfElementLocated(By.xpath("//div[contains(@class,'boton-firmado')]//span[contains(text(),'Firmado')]")));

        WebElement btnAutorizacion = getDriver().findElement(By.xpath("//p[contains(text(),'AUTORIZACIÓN')]"));
        JavascriptExecutor jsExecutor = (JavascriptExecutor) getDriver();
        jsExecutor.executeScript("arguments[0].scrollIntoView({block: 'center', inline: 'center'});", btnAutorizacion);

        List<WebElement> elementosFirmado = getDriver().findElements(By.xpath("//div[contains(@class,'boton-firmado')]//span[contains(text(),'Firmado')]"));

        if (elementosFirmado.isEmpty()) {
            return; //
        }
        boolean visible = elementosFirmado.get(0).isDisplayed();

        assertTrue("La etiqueta 'Firmado' NO debería estar visible en pantalla", !visible);
    }
}
